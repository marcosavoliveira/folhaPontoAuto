...
public class fillerFileForm extends JFrame {
    ...
    final String[] columnNames = {"Data", "Horário de Entrada", "Entrada no Intervalo", "Saída do Intervalo",
            "Horário de Saída", "Stand By"};
    ...
    public fillerFileForm(JFrame fillFile, int frameWidth, int frameHeight) throws ParseException {
        ...
        gravarButton.addActionListener(e -> {
            if (table1.getRowCount() == 0) {
                JOptionPane.showMessageDialog(null, "Valores não gerados, " +
                        "impossível efetuar gravação", "Erro", JOptionPane.ERROR_MESSAGE);
            }
            ...
            String baseDate = "01/" + (header.getMonth() + 1) + "/" + header.getYear();
            String filePath = JOptionPane.showInputDialog(null, "Caminho do Arquivo " +
                    "a ser atualizado: ");
            file.manipulateFile(footer, baseDate, filePath, (DefaultTableModel) table1.getModel());
        });
    }

    public static void main(String[] Args) throws ParseException {
        JFrame fillerFrame = new JFrame("Preenchedor Automático de Horas");
        fillerFrame.setContentPane(new fillerFileForm(fillerFrame, fillerFrame.getWidth(), fillerFrame.getHeight()).panel1);
        fillerFrame.pack();
        Utils.frameMethods frameMethods = new frameMethods();
        frameMethods.setterParamsFrame(fillerFrame, widthFrame, heightFrame);
    }
}
